...
const getName = (e: UploadEntry) =>
  isRealFile(e.file) ? e.file.name : e.file.name;
const getType = (e: UploadEntry) =>
  isRealFile(e.file) ? e.file.type : e.file.type || "";
const getSize = (e: UploadEntry) =>
  isRealFile(e.file) ? e.file.size : e.file.size ?? 0;
...
export type FileUploadComponentProps = {
  ...
  onFilesAdded?: (addedFiles: FileWithPreview[]) => void;
  onFilesChange?: (files: FileWithPreview[]) => void;
  /** Para arquivos do Storage: retorna URL (ex.: signed) para abrir/baixar. */
  onGetFileUrl?: (entry: FileWithPreview) => Promise<string | undefined>;
};
...
export default function FileUpload({
  ...
  const allSelected =
    selected.size > 0 && filtered.every((f) => selected.has(f.id));
  ...
  const resolveUrl = React.useCallback(
    async (entry: UploadEntry): Promise<string | undefined> => {
      if (onGetFileUrl) {
        const fw: FileWithPreview = {
          id: entry.id,
          file: entry.file as File | FileMetadata,
          preview: entry.preview,
        };
        return onGetFileUrl(fw);
      }
      return getPreviewUrl(entry) || undefined;
    },
    [onGetFileUrl],
  );

  const downloadOne = async (entry: UploadEntry) => {
  ...
  return (
    <div className="flex max-w-4xl flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-3">
          <h3 className="text-sm font-medium">
            Arquivos <span className="text-muted-foreground">({files.length})</span>
          </h3>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            <input
              ...
              placeholder="Buscar por nome, tipo ou extensão…"
              className="placeholder:text-muted-foreground border-input bg-background focus-visible:ring-ring h-8 w-56 rounded-md border px-7 text-[13px] outline-none focus-visible:ring-[2px]"
          ...
          <div className="flex items-center gap-1">
            ...
            <select
              id="sortby"
              className="bg-background border-input h-8 rounded-md border px-2 text-[13px]"
              value={sortBy}
      ...
      <div
        ...
        aria-label="Solte arquivos aqui ou use o botão para enviar"
      >
        <input
          {...getInputProps({
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div className="text-xs">
              <p className="font-medium">Solte arquivos para enviar</p>
              <p className="text-muted-foreground">
                Até {maxFiles} arquivos · {formatBytes(maxSize)} por arquivo
              </p>
            </div>
          </div>
          <Button variant="outline" size="sm" onClick={openFileDialog}>
            <UploadIcon
              className="-ms-1 size-3.5 opacity-60"
              aria-hidden="true"
            />
            Selecionar arquivos
          </Button>
            ...
            <div className="flex items-center gap-2 text-[13px]">
              <label className="inline-flex cursor-pointer items-center gap-1">
                ...
                <span className="text-muted-foreground">
                  {selected.size}/{filtered.length} selecionados
                </span>
              </label>
            </div>
            <div className="flex items-center gap-2">
              ...
              <Table>
                <TableHeader className="text-xs">
                  <TableRow className="bg-muted/50">
                    ...
                    <TableHead className="h-9 py-2">Tamanho</TableHead>
