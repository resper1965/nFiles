...
| Componente | Função |
|------------|--------|
| **Catálogo de padrões** | Lista de padrões disponíveis: (1) **Padrão seed** — ex.: INGREDION \| UNIMED NACIONAL \| TIPO DOC \| DESCRIÇÃO \| DATA (ver `regras-sugeridas-seed.md`); (2) **Padrões genéricos** — ex.: data no início, slug, numeração; (3) **Padrões customizados** — salvos pelo usuário. |
| **Escolha na UI** | Tela ou painel onde o usuário **seleciona** um padrão (dropdown, cards, etc.) ou **monta** um padrão a partir de campos (razão social, operadora, tipo doc, descrição, data). O padrão escolhido vira uma **regra** (ou template de regra). |
...
- **Escolha na UI:** seletor de padrão (dropdown) no card Regras; para o padrão "Seed completo", formulário opcional (Razão social, Operadora, Tipo doc, Descrição).
